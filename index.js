...
const ban_bot = require('./models/banned-bots.js')
...
client.once('ready', () => {
  client.user.setActivity("⚠️ The site is currently offline due to hosting provider switching. We hope to be online shortly. ⚠️", { type: 'WATCHING' });
...
const admin_user_ids = ["705114789374066748", "735813371433058354", "843131483597570068", "726481895961002065"]
module.exports.auth = async(req, res, next) => {
    ...
    try {
        ...
        if (admin_user_ids.includes(req.user.id) || member.roles.cache.has(process.env.WebMod))
            req.user.staff = true
    } catch(_) {}
...
const checkStaff = async (req, res, next) => { 
  ...
  const role = await guild.members.cache.filter(member => member.roles.cache.find(role => role.id === '933712802303066173')).map(member => member.id)
...
const checkAdmin = async (req, res, next) => { 
  const reqUser = req.user
  const guild = await client.guilds.cache.get(process.env.SERVERID)
  const role = await guild.members.cache.filter(member => member.roles.cache.find(role => role.id === '933712797223751690')).map(member => member.id)
...
app.post("/staffpanel/delete/:id", checkAuth, async (req, res) => {
  ...
  let botname = req.body.botname
	const denyEmbed = new Discord.MessageEmbed()
			.setTitle("Bot Denied:")
			.setDescription('Sadly your bot has been denied by our staff.')
			.addField("Bot: ", botname)
			.addField('Moderator: ', req.user.username + '#' + req.user.discriminator)
			.setFooter('Discord Lists | Bot Denied')
			.setColor('RED')
  await addbot.findOneAndRemove({
    botid: ID
  }).then(res.redirect("/staffpanel"))
    .then(client.channels.cache.get(process.env.LOGS_CHANNEL).send(denyEmbed))
})
...
// Redirecting...
app.get(['/addbot', '/add'], checkAuth, async function(req, res) {
  const login_logout = req.isAuthenticated()
  res.render('add', {
    login_logout,
  })
  ...
  (async () => {
    ...
    if (avatar === null) {
      ...
      avatar = `https://cdn.discordapp.com/avatars/${data.id}/${data.avatar}.png?size=128`
		...
		// Checks if bot is banned. If it is, it sends it to 403 page
		//if (ban_bot.findOne({ botid: req.body.id })) {
			//res.redirect('/error?code=403&message=Your bot was banned from our services.')
		//}
    ...
    let embed = new Discord.MessageEmbed()
      ...
      .setColor('YELLOW')
    client.channels.cache.get(process.env.LOGS_CHANNEL).send(embed);
    client.channels.cache.get(process.env.LOGS_CHANNEL).send('<@&933712802303066173>');
  })()
})
...
app.post('/stop-mainmode', (req,res)=>{
    ...
    if (value === "true") {
      rdb.set("MAINTENANCE", "false").then(() => {
        ...
        res.redirect("/error?code=400&message=MAINTENANCE mode is already turned off.")
    }
  })
})
...
app.get('/user/@me', checkAuth, async function(req, res) {
  ...
  //const guild = await client.guilds.cache.get(process.env.SERVERID)
  const admin_role = await guild.members.cache.filter(member => member.roles.cache.find(role => role.id === '933712797223751690')).map(member => member.id)
  console.log('Guild -' + guild)
  console.log(role)
...
app.get(['/yt', '/youtube'], async function(req, res) {
  res.redirect("https://www.youtube.com/channel/UCyx4PE3gfNByN0ZUwdB82RQ")
})

app.get('/support', async function(req, res) {
  res.redirect('https://discordlists.freshdesk.com/')
})
...
// IM trying to put the bot into lockdown if it gets raided
